...
function Log:set_level(level)
  local logger_ok, logger = pcall(function()
    return require("structlog").get_logger "vim_"
  end)
...
function Log:init()
  ...
  local log_level = Log.levels[(utils.log.level):upper() or "WARN"]
  local log_ = {
    vim_ = {
      sinks = {
        structlog.sinks.Console(log_level, {
          ...
          processors = {
            ...
            structlog.processors.StackWriter({ "line", "file" }, {
              max_parents = 0,
              stack_level = 2,
            }),
  ...
  log_.vim_.sinks[1].async = false -- HACK: Bug in structlog prevents setting async to false
  structlog.configure(log_)
  local logger = structlog.get_logger "vim_"

  -- Overwrite `vim.notify` to use the logger
  if vim_.log.override_notify then
...
function Log:get_path()
  return string.format("%s/%s.log", get_cache_dir(), "vim_")
end
